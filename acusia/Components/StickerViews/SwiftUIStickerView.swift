...
struct SwiftUIStickerView: View {
    // zIndex
    ...
    var body: some View {
        ...
        let doubleTap3D = TapGesture(count: 2)
            .onEnded {(value) in
                withAnimation(.spring(response: 0.4, dampingFraction: 0.8, blendDuration: 0)) {
                        if activeSticker == .swiftui {
                            activeSticker = nil
                        } else {
                            activeSticker = .swiftui
                        }
                }
        ...
        ZStack {
            Image("swiftuiStickerOutline")
                .resizable()
                .scaledToFill()
                .frame(width: 118, height: 118, alignment: .center)
                .offset(x: 0, y: 0)
                .foregroundColor(activeSticker == .swiftui ? .white.opacity(0.4) : .white)
                .aspectRatio(contentMode: /*@START_MENU_TOKEN@*/.fill/*@END_MENU_TOKEN@*/)
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(zAxisSliderValue) : .zero, axis: (x: 0, y: 0, z: 1))
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(xAxisSliderValue) : .zero, axis: (x: 1, y: 0, z: 0))
                .shadow(color: Color.black.opacity(dragTrigger ? 0.35 : 0.25), radius: dragTrigger ? 25 : 4, x: 0, y: dragTrigger ? 55 : 2)

           
            Image("swiftuiSticker")
                .resizable()
                .scaledToFill()
                ...
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(zAxisSliderValue) : .zero, axis: (x: 0, y: 0, z: 1))
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(xAxisSliderValue) : .zero, axis: (x: 1, y: 0, z: 0))
                .offset(x: 0, y: activeSticker == .swiftui ? -1*offsetSliderValue : 0)
            ...
            Image("swiftuiStickerOutline")
                ...
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(zAxisSliderValue) : .zero, axis: (x: 0, y: 0, z: 1))
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(xAxisSliderValue) : .zero, axis: (x: 1, y: 0, z: 0))
                .offset(x: 0, y: activeSticker == .swiftui ? -2*offsetSliderValue : 0)
            ...
            Rectangle()
                ...
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(zAxisSliderValue) : .zero, axis: (x: 0, y: 0, z: 1))
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(xAxisSliderValue) : .zero, axis: (x: 1, y: 0, z: 0))
                .offset(x: 0, y: activeSticker == .swiftui ? -3*offsetSliderValue : 0)
            ...
            Image("swiftuiSticker")
                ...
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(zAxisSliderValue) : .zero, axis: (x: 0, y: 0, z: 1))
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(xAxisSliderValue) : .zero, axis: (x: 1, y: 0, z: 0))
                .offset(x: 0, y: activeSticker == .swiftui ? -4*offsetSliderValue : 0)
            ...
            Image("NoiseLayer")
                ...
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(zAxisSliderValue) : .zero, axis: (x: 0, y: 0, z: 1))
                .rotation3DEffect(activeSticker == .swiftui ? .degrees(xAxisSliderValue) : .zero, axis: (x: 1, y: 0, z: 0))
                .offset(x: 0, y: activeSticker == .swiftui ? -5*offsetSliderValue : 0)
            ...
            Rectangle()
                ...
                .opacity(activeSticker == .swiftui ? 0 : 1)
                .mask {
                    Image("swiftuiStickerOutline")
                        .resizable()
                        .scaledToFill()
