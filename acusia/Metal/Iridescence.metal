...
#include "IridescenceShaderTypes.h"
...
[[stitchable]]
void iridescenceShader(realitykit::surface_parameters params,
                       constant IridescenceUniforms &uniforms)
{
    // Get world-space normal
    float3 N = normalize(params.geometry().normal());
    
    float3 V = normalize(uniforms.cameraPosition - params.geometry().world_position());
    
    // Use a fixed light direction (you can adjust this as needed)
    float3 L = normalize(float3(1.0, 1.0, 1.0));
    
    // Calculate half vector
    float3 H = normalize(V + L); // Assuming view direction is always (0,0,1) in tangent space
    ...
    float thickness = mix(uniforms.iridescenceThicknessMin,
                          uniforms.iridescenceThicknessMax,
                          uv.y); // Using UV.y instead of texture sampling
    ...
    params.surface().set_roughness(half(uniforms.roughness));
    params.surface().set_metallic(1.0h);
}
